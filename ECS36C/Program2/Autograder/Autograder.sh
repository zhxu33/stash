...
for exe in $(cat Config/SubmissionExecutables.txt); do
    ...
    for i in $(seq 1 $numtests); do
        if [ ! -f TestCases/TestCase$i.json ]; then
            echo "Error: missing test case Testcases/TestCase$i.json. Re-copy all grading files and re-run the script."
            exit
        fi
